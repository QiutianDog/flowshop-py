...
if __name__ == '__main__':
    ...
    for index, group in enumerate(groups, start=1):
        ...
        ax1.plot(x, y_res, "bo-", label="tss_res")
        ax1.set_ylabel("tss_res")

        ax2 = ax1.twinx()
        ax2.plot(x, y_time, "go--", label="tss_time")
        ax2.set_ylabel("tss_time")

    fig.legend(labels=("tss_res", "tss_time"), loc="center")

    # 保存图片
    plt.savefig(ROOT_DIR + "/data/tss_res.png", format="png", bbox_inches="tight")
    plt.show()
